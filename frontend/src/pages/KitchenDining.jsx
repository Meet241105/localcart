...
export default function KitchenDining() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        const data = await fetchProducts({ category: "kitchen-dining", limit: 24 });
        setClassy(data);
      } catch (error) {
  ...
  const displayClassy = classy.length ? classy : fallbackClassy;
  ...
  const filteredClassy = displayClassy.filter((item) => {
    const itemTag = toLabel(item.tag || item.badge);
    return activeTab === "All" || itemTag === activeTab;
  });
