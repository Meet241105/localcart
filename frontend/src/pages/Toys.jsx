...
export default function Toys() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        const data = await fetchProducts({ category: "toys", limit: 24 });
        setPopular(data);
      } catch (error) {
        console.error("Failed to load toys products:", error);
      }
    };
  ...
  const displayPopular = popular.length ? popular : fallbackPopular;
